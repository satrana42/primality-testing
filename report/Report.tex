...
\begin{algorithmic}
\Procedure{AKSProbabilisticPrimalityTest}{$n$} 

\State $r \gets min \{i: i \in \mathbb{N},\ i \leq \max(3,\ \lceil \log^{3+\frac{2}{k}} n \rceil),\ o_i(n) > \log^{1+\frac{1}{k}} n \}$ \Comment {Step 1}

\For {$a = 2$, $a \leq r$, $a \gets a+1$}
	\If {$1 < (a,n) < n$}
		\State \Return $false$  \Comment {Step 2}
	\EndIf
\EndFor

\If {$n \leq r$}
	\Return $true$  \Comment {Step 3}
\EndIf

\For {$a = 1$, $a \leq \lfloor \phi^\frac{1}{k+1}(r) \log n \rfloor$, $a \gets a+1$}
	\If {$(X+a)^n \not \equiv X^n + a \mod (n, X^r - 1)$}
		\State \Return $false$  \Comment {Step 4}
	\EndIf
\EndFor
\State \Return $true$ \Comment {Step 5}
\EndProcedure
\end{algorithmic}
\end{algorithm}
