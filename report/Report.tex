...
\While {$iter > 0$} \Comment {$iter$ is number of iterations}
	\State $a \gets \Call{Random}{1,n-1}$ \Comment {Random number in the range $[1,n-1]$}
...
\begin{algorithmic}
\Procedure{GeneralisedAKSPrimalityTest}{$n$, $S$}

\If {$n < 2$ or $n = a^b$ for $a, b \in \mathbb{N}$ and $b \geq 2$}
	\State \Return $false$  \Comment {Step 1}
\EndIf

\State $r \gets min \{i: i \in \mathbb{N},\ i \leq \lceil \log^{3+\frac{2}{k}} n \rceil,\ o_i(n) > \log^{1+\frac{1}{k}} n \}$ \Comment {Step 2}
...
\For {$a = 1$, $a \leq l_k$, $a \gets a+1$}
	\For {$m \in S$}
		\If {$(X+a)^m \not \equiv X^m + a \mod (n, X^r - 1)$}
			\State \Return $false$  \Comment {Step 5}
		\EndIf
	\EndFor
\EndFor
