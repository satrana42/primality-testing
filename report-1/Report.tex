...
If $n$ is a prime then Algorithm~\ref{algorithm:AKSPrimalityTest} returns $true$.
\end{lemma}
...
We have shown that Algorithm~\ref{algo:AKSPrimalityTest} returns $true$ if input $n$ is a prime. What remains to be shown is that it returns $false$ if $n$ is composite.
...
\begin{theorem}
If Algorithm~\ref{algo:AKSPrimalityTest} returns $true$ then input $n$ is prime.
\end{theorem}
\begin{proof}
