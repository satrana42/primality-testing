...
Since $X$ is a primitive $r^{th}$ root of unity in $\mathbb{Z}_n[X]/(X^r-1)$ and $(p,r) = 1$, $X^p$ is also a primitive $r^{th}$ root of unity in $\mathbb{Z}_n[X]/(X^r-1)$. Conversely, every primitive $r^{th}$ root of unity can be arrived at in this fashion. And therefore,
...
\begin{figure}
  ...
  \label{fig:aks_5000}
\end{figure}
Figure~\ref{fig:aks_5000} represents the values of $r$, $\log n$ and $l$ against $n$ along the $x$-axis, for $n \leq 5000$. It can be seen that $r \sim \log^2 n$, and therefore $l \sim r$ in this range. Also, both $r$ and $l$ increase roughly in a step-fuction fashion along with $\log n$. 
